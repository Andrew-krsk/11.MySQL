...
-- 2.1
SELECT s.shopname AS MurzikShopname FROM shops AS s
JOIN cats AS c
ON c.shops_id = s.id
WHERE c.name = 'Murzik';
-- 2.2
...
SELECT s.shopname 
FROM shops AS s
LEFT JOIN cats AS c ON s.id = c.shops_id AND (c.name = 'Murzik' OR c.name = 'Zuza')
